...
router.get('/base_elements', (request, response) => {
  ...
  var id_user = 1;
  var flag = true;
  client.query('SELECT * FROM Elements WHERE id_user = $1 and clipboard = $2;', [id_user, flag], (error, results) => {
    if (error) {
      response.status(404)
    }
...
router.delete('/base_elements/:id_element', (request, response) => {
  ...
  client.query('DELETE FROM Elements WHERE id_element =$1;', [id_element], (error, results) => {
    if (error) {
...
/*router.post('/base_elements', (request, response) => {
  ...
  client.query('INSERT INTO Elements (title_element, id_user, body_element, sourse) VALUES ($1, $2, $3, $4);', [title, id_user, image, sourse], (error, results) => {
    if (error) {
